...
class ProductController1 extends Controller
{
    ...
    public function all_product_user()
    {
        ...
        $product = product::with('images')->with('provider')->paginate(9);
        ...
        return view('client.category')->with('products', $product)->with('providers', $provider)->with('categories', $category)->with('min',$min)->with('max',$max);
    }
